...
export class AuthService {
  ...
  public fullName = new EventEmitter();
  
  public rutaAnterior: string;

  constructor(private _http: HttpClient, private _cripto: CriptoService) {
  ...
  login(correo: string, password: string) {
    const datos =
      'grant_type=password&username=' + correo + '&password=' + password;
    //const datos = 'grant_type=password&username=' + this._cripto.encrypt(correo) + '&password=' + this._cripto.encrypt(password);

    return this._http.post(this.url + '/Token', datos).pipe(
      map((response) => {
        this.guardarToken(response['access_token'], response['expires_in']);
        this.fullName.emit(response['fullName']); //Emite el nombre completo del usuario cuando llega el 'response'
        ...
        const idEmpresaEncriptado = this._cripto.encrypt(response['idEmpresa']);
        // const idEmpresaEncriptado = this._cripto.encrypt(response['idEmpresa']);
        const idTipoUsuarioEncriptado = this._cripto.encrypt(
          response['usertypeid']
        );
  ...
  estaAutenticado(): boolean {
    ...
    if (expiraDate > new Date()) {
      //Si la fecha de expiracion no ha pasado de la fecha actual
      return true;
    } else {
      return false;
    }
  ...
  setRutaAnterior(previousUrl)
  {
    this.rutaAnterior = previousUrl;
  }
